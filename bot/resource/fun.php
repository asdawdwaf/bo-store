...
function selectbase($message){
	...
	$menu =  ['inline_keyboard' => [
		...
		[['text'=>"🇧🇷 CC's do Brasil",'callback_data'=>"select_br"]],
		[['text'=>"🇲🇾 CC's da Gringa",'callback_data'=>"select_gringa"]],
		[['text'=>"🔙 Voltar",'callback_data'=>"volta_loja"]]

	,]];


	$edit = bot("editMessageText",array( "message_id" => $message['message_id'] , "chat_id"=> $chat_id , "text" => "*Você selecionou as:*","reply_markup" =>$menu,"reply_to_message_id"=> $message['message_id'],"parse_mode" => 'Markdown'));
	if (!$edit){
...
function select($message ,$query, $opt){
	...
	bot("answerCallbackQuery",array("callback_query_id" => $idquery , "text" => "Foi selecionada as {$opt} , desejamos boas compras.","show_alert"=> false,"cache_time" => 10));
	...
	$menu =  ['inline_keyboard' => [
		...
		[['text'=>"Continuar comprando!",'callback_data'=>"ccun"]]

	,]];

	if ($t){
		bot("editMessageText",array( "message_id" => $message['message_id'] , "chat_id"=> $chat_id , "text" => "*Parabéns! Foi selecionado as $opt\nCaso mude de ideia, troque pelo comando /country e selecione outros tipos.*" ,"reply_markup" => $menu,"reply_to_message_id"=> $message['message_id'],"parse_mode" => 'Markdown'));
	}
